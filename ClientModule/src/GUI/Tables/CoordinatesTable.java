...
public class CoordinatesTable extends AbstractPageShower implements TableNavigation {
    ...
    private CoordinatesTableModel tableModel;
    private JPanel jp;
    private CardLayout cl;

    private CommandsPage comPage;

    CoordinatesTable(CardLayout cardLayout, JPanel jPanel) {
        cl = cardLayout;
        jp = jPanel;
        tableModel = new CoordinatesTableModel();
        ...
        jTable.addMouseListener(new ClickMouseAdapter() {
            ...
            protected void fireSingleClick(ActionEvent e, Point p) {
                ...
                filter(x, ((CoordinatesTableModel) jTable.getModel()).getNotFormattedValue(y, x));
            }
            ...
            protected void fireCenterClick(ActionEvent e, Point p) {
                ...
                try {
                    Ticket t = ((CoordinatesTableModel) tableModel).getTicketByCoordinates(id);
    ...
    private Ticket getTicket(int id) {
        try {
            return ((CoordinatesTableModel) tableModel).getTicketByCoordinates(id);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
